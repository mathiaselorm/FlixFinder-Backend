...
"""
        def get_object_by_id_or_slug(model, identifier, id_field='id', slug_field='name'):

    Retrieve a model instance by its ID or name.

    :param model: The model class.
    :param identifier: The identifier, which can be either an ID or a name.
    :param id_field: The field name for the ID (default is 'id').
    :param slug_field: The field name for the name (default is 'name').
    :return: The model instance or raise Http404 if not found.

    if identifier.isdigit():
        # Tries to fetch by ID
        filter_args = {id_field: identifier}
    else:
        # Assumes it's a name if not a digit
        filter_args = {slug_field: identifier}
