...
class Genre(models.Model):
    name = models.CharField(max_length=100, unique=True, help_text="Enter a unique name for the genre.")
    tmdb_id = models.IntegerField(unique=True, null=True, help_text="The associated TMDB ID for the genre.")
    slug = models.SlugField(max_length=100, unique=True, blank=True, help_text="URL-friendly identifier automatically generated from the name.")
...
class Movie(models.Model):
    ...
    tmdb_id = models.IntegerField(unique=True, null=True, blank=True, help_text="TMDB identifier for the movie.")
    title = models.CharField(max_length=255, help_text="The title of the movie.")
    slug = models.SlugField(max_length=255, unique=True, blank=True, db_index=True, help_text="URL-friendly identifier automatically generated from the title.")
    overview = models.TextField(blank=True, null=True, help_text="Brief description of the movie.")
    release_date = models.DateField(blank=True, null=True, help_text="The release date of the movie.")
    ...
    average_rating = models.DecimalField(max_digits=4, decimal_places=2, default=0.0, db_index=True, help_text="Calculated average rating based on user reviews.")
    ...
    def update_average_rating(self):
        cache_key = f"{self.pk}_average_rating"
        average = cache.get(cache_key)
        if average is None:
            average = self.ratings.aggregate(average_score=Avg('score'))['average_score'] or 0.00
            cache.set(cache_key, average, timeout=3600)  # Cache for 1 hour
...
class Watchlist(models.Model):
    ...
    def __str__(self):
        return f"{self.user.get_full_name()}'s watchlist"
